...
function PokemonProfile() {
  ...
  const renderImg = () => {
    if (pokemon.length === 0) {
      ...
      return <img className="w-37 tablet:w-50 tablet:justify-self-center" alt={pokemon.name} src={pokemon.sprites.front_default} />;
    }
  };
  ...
  return (
    <div className="grid grid-cols-2 tablet:grid-cols-1">
      ...
      <div className="flex flex-col text-left place-self-center tablet:gap-y-2 tablet:text-center">
        {!contextValue.loading ? <p className="font-bold font-minecraft text-2xl tablet:text-3xl capitalize">{pokemon.name}</p> : <div className="h-5 w-1/2 rounded bg-grey-40 animate-pulse"></div>}
