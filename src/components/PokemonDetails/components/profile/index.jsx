...
function PokemonProfile() {
  ...
  const renderImg = () => {
    if (pokemon.length === 0) {
      return <div className="w-37 h-37 rounded skeleton animate-pulse  tablet:w-50 tablet:h-50 tablet:justify-self-center  tablet:mt-12"></div>;
    } else {
  ...
  return (
    <div className="grid grid-cols-2 tablet:grid-cols-1">
      ...
      <div className="flex flex-col text-left place-self-center tablet:gap-y-2 tablet:text-center">
        {!ctxValue.loading ? <p className="font-bold font-minecraft text-2xl tablet:text-3xl capitalize">{pokemon.name}</p> : <div className="h-5 w-full rounded skeleton animate-pulse"></div>}
